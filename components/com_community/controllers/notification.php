		...
		foreach( $rows as $row )
		{
			if(!in_array($row->id,$friendIdList)){
				...
				$obj->action  =
					'
					<b>
						<a onclick="jax.call(\'community\' , \'friends,ajaxApproveRequest\' , \''.$row->connection_id.'\');" class="cButton cButton-Small cButton-Green" href="javascript: void(0)">'.JText::_('COM_COMMUNITY_PENDING_ACTION_APPROVE').'</a>
						<a onclick="jax.call(\'community\',\'friends,ajaxRejectRequest\',\''.$row->connection_id.'\');" class="cButton cButton-Small" href="javascript:void(0);"><i class="cWindow-X"></i></a>
					</b>';
				$data[] = $obj;
			}
