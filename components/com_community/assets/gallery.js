joms.extend({
	gallery:{
		...
		createPhotoTag: function(tag){
			...
			// Create photo tag
			var newPhotoTagcreatePhotoTags = new Array();
			joms.jQuery.each(tag, function(i, tag)
			{
				...
				photoTag.data('tag', tag)
						...
						.hover(
							function(){ joms.gallery.showPhotoTagNew(tag.id, 'Label'); },
							function(){ joms.gallery.hidePhotoTagNew(tag.id); }
						)
                        .click(
                            function(){ alert("OK"); }
                        )
				...
				var photoTagLabel = joms.jQuery('<div class="photoTagLabel show">');

				photoTagLabel.html('<i>'+tag.displayName+'</i>');
		...
		drawPhotoTag: function( tag, photo){
			...
			// Create photoTag
			var photoTag = joms.jQuery('<div class="photoTag showLabel">');
		...
		addPhotoTag: function(userId){
            ...
            var text = joms.jQuery('.selected span').text();
        ...
        showPhotoTagNew: function(id, classSuffix){
            //Bien Aqui para mostrar el HOVER
        },
        hidePhotoTagNew: function(id){
            //QUitar JHover
        },
		createPhotoTextTag: function(tags){
			...
			joms.jQuery.each(tags, function(i, tag)
			{
				...
				photoTextTag.data('tag', tag)
							...
							.hover(
								function(){ joms.gallery.showPhotoTagNew(tag.id, 'Force'); },
								function(){ joms.gallery.hidePhotoTagNew(tag.id); }
							 )
                            .click(
                                function(){ joms.gallery.hidePhotoTagNew(tag.id); }
