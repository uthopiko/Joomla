...
class CPhotoTagging
{
	...
	public function addTag( $tagObj )
	{
		...
		if($tagModel->isTagExists($tagObj->photoId, $tagObj->userId) && $tagObj->userId != 0)
		{
			$this->_error	= JText::_('COM_COMMUNITY_PHOTO_TAG_EXIST');
			return 0;
		}
		...
		if($tagModel->customAddTag($tagObj->photoId , $tagObj->userId , $position, $tagObj->text)->return_value['addTag'])
		{						
