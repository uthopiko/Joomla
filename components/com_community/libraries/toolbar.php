...
class CToolbarLibrary
{
	...
	public function getHTML($userId = '')
	{
		...
		if ( ! $html)
		{
			...
			$format	= $jinput->get->get('format', 'html', 'STRING'); //JRequest::getVar('format', 'html', 'get');
			...
			$groupMiniHeader = '';
			$task            = $jinput->get('task', ''); //JRequest::getVar('task', '');
	...
	{
		$url      = 'index.php?';
		$segments =  $_GET; // @TODO Doesn't see the benefit of assign by reference here.
