...
class CommunityViewVideosHelper extends CommunityView
{
	...
	public function _getVideosHTML( $videos )
	{
		...
		$tmpl	= new CTemplate();
		$tmpl->set( 'sort'				, $jinput->get('sort', 'latest', 'STRING') ); //JRequest::getVar('sort', 'latest') );
	...
	public function search()
	{
		...
		$search		= $jinput->request->get('search-text' , '', 'STRING'); //JRequest::getVar( 'search-text' , '' , 'REQUEST' );
		$result		= array();
		$pagination	= null;
		$total		= 0;
	...
	public function _userVideo()
	{
		...
		$my			= CFactory::getUser();
 		$model		= CFactory::getModel('videos');
 		$videoId	= $jinput->get('videoid' , '', 'INT'); //JRequest::getVar('videoid' , '' , 'GET');
